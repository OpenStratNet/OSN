RssFeed.publish( 'updates', function() {
  ...
  feed.setValue( 'lastBuildDate', moment().format('ddd, DD MMM YYYY hh:mm:ss ZZ') );
  feed.setValue( 'pubDate', moment().format('ddd, DD MMM YYYY hh:mm:ss ZZ') );
  feed.setValue( 'ttl', 1 );
  ...
  posts.forEach( function( post ) {
    feed.addItem({
      title: post.title,
      description: post.cleanDescription, //cleanDescription ?
